import { ListSubheader, List, Box } from '@mui/material';
...
const reduceChildRoutes = ({
  ...
  if (item.items) {
    ...
    ev.push(
      ...
      ev.push(<Box hidden={item.hidden}>
      <SidebarMenuItem
        key={key}
        active={exactMatch}
        name={item.name}
...
function SidebarMenu() {
    ...
    const [menus, setMenus] = useState<MenuItems[]>(menuItems);
 
    return (
    <>
            {menus?.map((section) => (
                <MenuWrapper hidden={section.hidden}
              key={section.heading}
              subheader={
